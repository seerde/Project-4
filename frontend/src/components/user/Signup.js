...
const Signup = (props) => {
    ...
    let registerHandler = async (e) => {
        ...
        if (register["password"] == register["password_confirmation"]) {
            try {
                let data = await axios.post("http://localhost:3001/api/auth/signup", register);
                if (data.data.token) {
        ...
        <div className="container">
            <div className="inner-container-m">
                <div className="box">
                    ...
                    <div className="input-group">
                        <label htmlFor="password"></label>
                        <input type="password" name="password_confirmation" placeholder="Password confirmation " onChange={changeHandler} />
                    </div>


                    <button type="button" className="btn btn-sm" onClick={(e) => registerHandler(e)} >Sign Up</button>
                </div>
